...
export class Service {
    ...
    notices: any;
    constructor(private reqhttp: HTTP, 
        private http: Http, 
            ...
            this.http.get(this.config.url + '/wp-admin/admin-ajax.php?action=mstoreapp-keys', this.config.options).map(res => res.json()).subscribe(data => {
                ...
                this.http.get(this.config.setUrl('GET', '/wc-api/v3/products/categories?', false), this.config.options).map(res => res.json()).subscribe(data => {
                    ...
                    for (var i = 0; i < this.categories.product_categories.length; i++) {
                        if (this.categories.product_categories[i].parent == '0' && this.categories.product_categories[i].display != 'both') {//Condicional extra para filtrar categorias
    ...
    getNotices(id) {
        return new Promise(resolve => {
            this.http.get(this.config.url + '/wp-json/wp/v2/posts?categories='+id).map(res => res.json()).subscribe(data => {
                this.notices = data;
    ...
    getCategories() {
        return new Promise(resolve => {
            this.http.get(this.config.url + '/wp-json/wp/v2/categories?').map(res => res.json()).subscribe(data => {
                this.categories = data;
                resolve(this.categories);
            });
        });
